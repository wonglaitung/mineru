...
def expand_query_for_financial_analysis(query: str) -> list[str]:
    ...
    Returns:
        扩展后的关键词列表（繁体中文）
    """
    prompt = f"""你是一个财务分析专家，正在帮助用户从港铁公司财务报告中检索数据。

用户查询：{query}

请返回需要在文档中检索的关键词。注意：
1. 使用繁体中文
2. 关键词必须是文档中实际出现的词汇，不要用抽象概念
3. 用逗号分隔，只返回关键词

港铁财务报告常用词汇示例：
- 現金流量表用词：經營活動所得的現金淨額, 投資活動耗用的現金淨額, 融資活動所得的現金淨額
- 损益表用词：香港車務營運收入, 總收入, 營業成本, 期內利潤, 淨利潤, 公司股東應佔淨利潤
- 资产负债表用词：總資產, 總負債, 淨資產, 總權益, 現金銀行結餘及存款
