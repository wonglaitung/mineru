...
@app.post(
    path="/convert2markdown",
    status_code=200,
    summary="Synchronously parse files from base64-encoded content",
    description=(
        "Accept JSON body with base64-encoded file content, parse synchronously, "
        "and return results keyed by file index."
    ),
)
async def convert2markdown(
    http_request: Request,
    background_tasks: BackgroundTasks,
):
    body = await http_request.json()

    files_b64: list[str] = body.get("files", [])
    if not isinstance(files_b64, list):
        raise HTTPException(status_code=400, detail="files must be a list of base64 strings")
    if len(files_b64) == 0:
        raise HTTPException(status_code=400, detail="files must not be empty")

    lang_list: list[str] = body.get("lang_list", ["ch"])
    backend: str = body.get("backend", "hybrid-auto-engine")
    parse_method: str = body.get("parse_method", "auto")
    formula_enable: bool = body.get("formula_enable", True)
    table_enable: bool = body.get("table_enable", True)
    image_analysis: bool = body.get("image_analysis", True)
    server_url: Optional[str] = body.get("server_url", None)
    return_md: bool = body.get("return_md", True)
    return_middle_json: bool = body.get("return_middle_json", False)
    return_model_output: bool = body.get("return_model_output", False)
    return_content_list: bool = body.get("return_content_list", False)
    return_images: bool = body.get("return_images", False)
    response_format_zip: bool = body.get("response_format_zip", False)
    return_original_file: bool = body.get("return_original_file", False)
    start_page_id: int = body.get("start_page_id", 0)
    end_page_id: int = body.get("end_page_id", 99999)

    if response_format_zip:
        raise HTTPException(
            status_code=400,
            detail="response_format_zip is not supported for /convert2markdown",
        )

    validate_public_http_client_request(
        public_bind_exposed=bool(
            getattr(http_request.app.state, "public_bind_exposed", False)
        ),
        allow_public_http_client=bool(
            getattr(http_request.app.state, "allow_public_http_client", False)
        ),
        backend=backend,
        server_url=server_url,
    )
    ...
    try:
        ...
        for idx, file_b64 in enumerate(files_b64):
            if not isinstance(file_b64, str):
                raise HTTPException(
                    status_code=400,
                    detail=f"Each item in files must be a base64 string, got index {idx}",
                )

            try:
                file_bytes = b64decode(file_b64)
            except Exception as exc:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid base64 at index {idx}: {exc}",
                )

            filename = f"file_{idx}.pdf"
            normalized_stem = normalize_task_stem(Path(filename).stem)
            os.makedirs(uploads_dir, exist_ok=True)
            destination = build_upload_destination(uploads_dir, filename)
        ...
        file_names = [u.stem for u in stored_uploads]
        upload_paths = [u.path for u in stored_uploads]
        upload_names = [u.original_name for u in stored_uploads]
        ...
        for idx, original_name in enumerate(upload_names):
            stem = file_names[idx]
            ordered_results[str(idx)] = results.get(stem, {})
